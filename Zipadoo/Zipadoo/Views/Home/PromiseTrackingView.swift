...
struct PromiseTrackingView: View {
    ...
    @State var promiseViewModel: PromiseViewModel
    ...
    var body: some View {
        ...
        if let loginUserID = currentUser?.id {
            ...
            let filteredPromises = promiseViewModel.fetchPromiseData.filter { promise in
                return loginUserID == promise.makingUserID || promise.participantIdArray.contains(loginUserID)  
            }
            ScrollView {
                ForEach(promiseViewModel.fetchPromiseData, id: \.self) { promise in
                    NavigationLink {
